// Novo Checkout TIC
// Este script efetua o fluxo de nova venda de 1P vários produtos

describe('checkout multiplos produtos', () => {
    beforeEach(() => cy.visit('https://tic.enesolucoes.com.br/para-voce/telefonia-fixa'))
    it('telefinia-fixa' , function() {
        ...
        // cy.visit('https://tic.enesolucoes.com.br/para-voce/telefonia-fixa')
        cy.get('.flex > :nth-child(6) > .link').click()
        cy.get(':nth-child(2) > .b--light-gray > .mh6').click()
        cy.get('.mh20 > .Grid > .bt > .h-100 > .style-module--buttonWrapper--3FrCc > .w-100 > .pv12').click()
        cy.wait(6000)
        cy.get('.pv12 > .flex > span').click() 
        cy.log('════ FLUXO DE LOGIN ════')
        cy.get('#username').type('08796331666')
        cy.get('#password').type('ctbc2016{enter}')
        cy.wait(25000)
        cy.location().should((loc) => {
           expect(loc.toString()).to.eq('https://tic.enesolucoes.com.br/novo-checkout/migracao')
        })
        cy.get('.justify-end > .fw6').click()
